...
var Namespaces = require('./Namespaces.cjs');
...
class LogBase {
  ...
  constructor (name, opts = {}) {
    ...
    this.opts = opts;
    this._enabled = {};
    this.formatter = new Format.Format(opts);
  ...
  enable (namespaces = this.opts.namespaces) {
    const namespace = new Namespaces.Namespaces(namespaces);
    this._enabled = {}; // reset
    const level = namespace.isEnabled(this.name, this.opts.level);
    const active = level
      ? utils.LEVELS[utils.adjustLevel(level, utils.INFO)]
      : [];
    utils.LEVELS.TRACE.forEach(level => {
      const nlevel = this.opts.levelNumbers ? utils.toNumLevel(level) : level;
      const llevel = level.toLowerCase();
      if (active.includes(level)) {
        this._enabled[level] = true;
        this[llevel] = level === utils.FATAL
          ? (fmt, ...args) => {
    ...
    const nLOG = this.opts.levelNumbers ? utils.toNumLevel(utils.LOG) : utils.LOG;
    this.log = (fmt, ...args) => this._log(nLOG, fmt, args);
  }

  get enabled () {
    return this._enabled._cache || (
      this._enabled._cache = utils.LEVELS.TRACE.reduce((o, level) => {
