...
var serializers = require('./serializers.cjs');
 ...
 */
class LogEcs extends node.Log {
  /**
   * @param {string} name logger namespace
   * @param {LogOptionsEcs} opts
   */
  constructor (name, opts) {
    const { serializers: serializers$1, ..._opts } = opts || {};
    super(name, {
      ..._opts,
      timestamp: 'iso'
    });
    this.serializers = { ...serializers.ecsSerializers, ...serializers$1 };
  ...
  _toJson (obj, serializers) {
    ...
    return node.stringify(ecsObj)
  }
}

LogEcs.serializers = serializers.ecsSerializers;

exports.LogEcs = LogEcs;
