...
var LogBase = require('./LogBase.cjs');
var wrapConsole = require('./wrapConsole.cjs');
var wrapDebug = require('./wrapDebug.cjs');
var Sonic = require('./Sonic.cjs');
...
const options = {
  ...
  serializers: { err: err.errSerializer } // serializers definition
};

class Log extends LogBase.LogBase {
  /**
  ...
  constructor (name, opts) {
    Object.assign(
      options,
      utils.inspectOpts(process.env),
      utils.inspectNamespaces(process.env)
    );
    ...
    this.color = utils.selectColor(name, colorFn);
    this.levColors = utils.levelColors(colorFn);
    // noop for TS
    this.opts = {
    ...
    this.stream = this.opts.sonic
      ? Sonic.sonicStreams.use(this.opts.stream, {
    ...
    if (!this.opts.json) {
      ...
      this.formatter = new Format.Format(this.opts);
  ...
  static save () {
    Log.reset();
    utils.saveOpts(process.env, options);
  }
  ...
  static wrapConsole (name = 'console', opts) {
    ...
    return wrapConsole.wrapConsole(log, opts)
  }
  ...
  static wrapDebug () {
    return wrapDebug.wrapDebug(Log)
  }
  ...
  _logJsonColor (level, fmt, args) {
    ...
    str = str
      ...
      .replace(/"level":\s?(\d+)/, (m, level) =>
        this._color(m, this.levColors[utils.fromNumLevel(Number(level))], true)
      )
